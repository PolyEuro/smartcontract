...
contract erc20tokenRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. erc20tokens to distribute per block.
        uint256 lastRewardTime; // Last timestamp that erc20tokens distribution occurs.
        uint256 accerc20tokenPerShare; // Accumulated erc20tokens per share, times 1e18. See below.
        bool isStarted; // if lastRewardTime has passed
        uint16 depositFeeBP; // Deposit fee in basis points
        uint256 startTime;
    }

    address public erc20token = address(0x0000000000000000000000000000000000000000);

    // BALL tokens created per second.
    ...
    uint256 public rewardHalvingRate = 9000;
    ...
    //rewardperSecond 165343915340000000
    constructor(
        address _erc20token,
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "erc20tokenRewardPool: caller is not the operator");
        _;
    }
        ...
        if (halvingChecked) {
            halvingChecked = false;
                ...
                if (now >= nextHalvingTime) {
                    massUpdatePools();
                    rewardPerSecond = rewardPerSecond.mul(rewardHalvingRate).div(10000); // x90% (10% decreased every week)
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (now > pool.lastRewardTime && lpSupply != 0) {
            ...
            if (totalAllocPoint > 0) {
                uint256 _erc20tokenReward = _time.mul(rewardPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
                accerc20tokenPerShare = accerc20tokenPerShare.add(_erc20tokenReward.mul(1e18).div(lpSupply));
            }
        }
        return user.amount.mul(accerc20tokenPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _time = now.sub(pool.lastRewardTime);
            uint256 _erc20tokenReward = _time.mul(rewardPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            pool.accerc20tokenPerShare = pool.accerc20tokenPerShare.add(_erc20tokenReward.mul(1e18).div(lpSupply));
        }
        pool.lastRewardTime = now;
    }
    ...
    function _harvestReward(uint256 _pid, address _account) internal {
        ...
        if (user.amount > 0) {
            PoolInfo storage pool = poolInfo[_pid];
            uint256 _claimableAmount = user.amount.mul(pool.accerc20tokenPerShare).div(1e18).sub(user.rewardDebt);
            if (_claimableAmount > 0) {
                IBasisAsset(erc20token).mint(_account, _claimableAmount);
    ...
    function deposit(uint256 _pid, uint256 _amount) public lock checkHalving {
        ...
        user.rewardDebt = user.amount.mul(pool.accerc20tokenPerShare).div(1e18);
