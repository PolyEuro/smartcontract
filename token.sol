...
contract BTOKEN is ERC777OwnableUpgradeSafe, ContractGuard {
    using SafeMath for uint256;
    ...
    // TOTAL MAX SUPPLY = 100,000,000 TOKEN
    uint256 public constant COMMUNITY_FUND_POOL_ALLOCATION = 0 ether;
    uint256 public constant DEV_FUND_POOL_ALLOCATION = 0 ether;
    uint256 public constant IDO_ALLOCATION = 30000 ether;

    uint256 public constant VESTING_DURATION = 150 weeks;
    uint256 public constant startTime = 1624080000; 
    uint256 public constant endTime = startTime + VESTING_DURATION; 
    ...
    function initialize(uint256 _cap,address _taxFund) public initializer {
        ...
        taxFund = _taxFund;
        devFund = msg.sender;
        jackpotFund = msg.sender;
        jackpotFund = msg.sender;
        burnRate = 2; // 0.2%
        taxRate = 0; // 0.3% loyaltypool when open
        jackpotRate = 3;//0.3% 
        fundRate = 2;//0.2% 
        ...
        _mint(_msgSender(), IDO_ALLOCATION.add(1 ether), "", ""); // mint extra 1 MTokenERC777 for initial pools deployment
    }
   ...
   function setJackpotFund(address _jackpotFund) external onlyOwner {
    ...
    function transferFrom(
        ...
        require(recipient != address(0), "MTokenERC777: transfer to the zero address");
        require(holder != address(0), "MTokenERC777: transfer from the zero address");
        ...
        _approve(holder, spender, allowance(holder, spender).sub(amount, "MTokenERC777: transfer amount exceeds allowance"));
    ...
    function _send(
        ...
        require(from != address(0), "MTokenERC777: send from the zero address");
        require(to != address(0), "MTokenERC777: send to the zero address");
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedToFee[to]) {
            {
                ...
                if (_taxRate > 0) {
                    ...
                    _balances[from] = _balances[from].sub(_taxAmount, "MTokenERC777: transfer amount exceeds balance");
            ...
            }
            {
             ...
             {
                uint256 _fundRate = fundRate;
                if (_fundRate > 0) {
                    uint256 _devAmount = amount.mul(_fundRate).div(10000);
                    address _devFund = devFund;
                    _balances[from] = _balances[from].sub(_devAmount, "MTokenERC777: transfer amount exceeds balance");
                    _balances[_devFund] = _balances[_devFund].add(_devAmount);
                    _amount = _amount.sub(_devAmount);
                    emit Transfer(from, _devFund, _devAmount);
                }
            }
        }

        _balances[from] = _balances[from].sub(_amount, "MTokenERC777: transfer amount exceeds balance");
